...
public class CrudController {
    ...
    //批量导入
    @ResponseBody
    @RequestMapping(value = {"/b4/{splitDBName}"}, method = RequestMethod.GET)
    public Object batch4(@PathVariable("splitDBName") String splitDBName, String title) {
        List<TradeModel> list = new ArrayList<TradeModel>();
        long tid = System.currentTimeMillis();
        for (int i = 0; i < 110; i++) {
            TradeModel model = new TradeModel();
        ...
        task.setEntities(list);
        return tradeService.batchInsertTrade4(task);
    }
